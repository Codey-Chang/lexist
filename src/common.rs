...
use sudachi::{analysis::{stateful_tokenizer::StatefulTokenizer, stateless_tokenizer::StatelessTokenizer, Tokenize}, config::{Config, ConfigBuilder}, dic::{build::DictBuilder, dictionary::JapaneseDictionary, grammar::Grammar, header::Header, lexicon::Lexicon, storage::{Storage, SudachiDicData}, subset::InfoSubset, DictionaryLoader}, error::SudachiResult, prelude::{Mode, MorphemeList}};
...
impl SfTokenizer {
    ...
    pub fn builder(system: &[u8]) -> TokenizerBuilder {
        TokenizerBuilder {
            system,
            user: Vec::new(),
            conn: None,
